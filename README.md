...
3. **Attach (kar topu, çekirdek kanıt kapılı)** — Dosyadaki HER alarm, zaman penceresi çakışan olaylara teklif edilir; en yakın nedensel eve gider (blame eşleşmesi > aynı servis > graf mesafesi; eşitlikte "o an en sıcak" olay kazanır). Bir alarm kartlara YALNIZCA nedensel kanıtla bağlanır: mesajının blame hedefi olayda olması, veya anomali/sert-hata-işaretçisi/kritik-seviye (sev≥4) bir alarmın olay servisiyle ya bağımlılık komşuluğunda bulunması. "Aynı servis" veya "graf yakınlığı" tek başına arka plan gürültüsünün geniş olay pencerelerindeki en kolay kılığıdır — bu yüzden tek başına bağlama yetkisi yoktur (`not_core_evidence` gerekçesiyle kasıtlı olarak elenir). Pencere de yalnız çekirdek kanıtla büyür; 3 süpürme yapılır çünkü pencere geç büyür ve kuyruk alarmları yeniden teklif edilmelidir.
4. **Kök neden skorlaması** — Her olayın adayları beş sinyalle sıralanır: blame sayısı, sert hata işaretçileri (kök-tipli ağırlıklı), **grafın açıklama gücü** (bozulduğunda kaç etkilenen servis etkilenir), erkenlik, kritiklik. DB katmanı indirimi: servis kendi DB'si kırıyorken servisin DB-katmanı hataları yarıya iner (türev sayılır). Ağ işaretçileri tek rack'e yoğunlaşmışsa kök **rack'in kendisi** olur (top-of-rank switch). Tek servisli tohumlarda (yavaş sızıntı, dış sağlayıcı) karşı-olasılık adayları etkilenen servislerden ölçümlü kanıtla tamamlanır — her kart karşı-olasılıksız kalmaz.
5. **Gürültü denetimi** — Olaya bağlanmayan HER alarm kaydı, açık gerekçeyle (zaman çakışması yok / anomali eşiği altında / topolojik bağ yok / nedensel kanıt yok) elenir. Hiçbir alarm sessizce kaybolmaz.

**Sonuç (bu veriyle):** 5 olay kartı — session-service bellek sızıntısı — yavaş gelişen (234 alarm), dc1/rack-A ağ kesintisi (459), billing-db disk/tablespace doluluğu (276), payment-provider-gw dış sağlayıcı degradasyonu (168), subscriber-db havuz tükenmesi (164). 1699 alarm gerekçeli gürültü. Zaman içinde çakışan iki bağımsız olay (session sızıntısı ↔ ödeme sağlayıcı) **bilinçli olarak ayrı tutuldu**; tüm sev5 alarmlar kartlara bağlıdır.
...
- **Bilinen sınırlar (dürüstçe):** (1) Zaman içinde çakışan olaylarda tek tek alarm atfı belirsizlik taşır (ör. mobile-bff hem sızıntı hem ödeme olayının mağduru); kart ayırımı doğru olsa da alarm bazlı atıfta hata payı vardır. (2) Kök neden skorlamasının ağırlıkları (`blame=1.0, marker=3.0...`) veriye göre kalibre edilmiştir; farklı senaryoda yeniden ayar gerektirebilir — tümü CLI argümanına bağlıdır. (3) Doğrulama verisi (gerçek olay etiketleri) kapalı olduğu için kök neden isabet oranı ölçülemedi; her hipotez karşı-olasılıklarıyla birlikte sunuldu. (4) "Benzer geçmiş olay örüntüsü" bonusu verilere dayalı bir eşleştirme üretmiyor: kartlar arası benzerlik imzası hesaplanır ancak gerçek bir geçmiş olay arşivi olmadığından bu alan boş bırakılır.
